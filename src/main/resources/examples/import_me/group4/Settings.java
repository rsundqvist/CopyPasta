...
public abstract class Settings {
    ...
    public static Map<String, String[]> getAbout () {
        ...
        about.put(first_run, // Key
                  new String[]{"First Run", // Display name
                      "Indicates that the program is running for the first time, showing startup help and overriding previous settings. MAY CAUSE DATA LOSS!!", Boolean.class.getCanonicalName() + ""});
    ...
    public static void loadFromProperties () {
        ...
        try {
            ...
            String s = (String) properties.get(startup_version_check); if (s != null && !s.isEmpty())
                STARTUP_VERSION_CHECK = Boolean.parseBoolean(s);

            s = (String) properties.get(first_run); if (s != null && !s.isEmpty())
    ...
    public static void putToProperties () {
        ...
        putValue(startup_version_check, "" + STARTUP_VERSION_CHECK); putValue(first_run, "" + FIRST_RUN);
    }
    // endregion
    ...
    public static void restartForSettingsEffect () {
        Alert alert = new Alert(Alert.AlertType.INFORMATION); alert.setHeaderText("Settings saved");
        alert.setContentText("The new settings will take effect once you restart the program."); alert.showAndWait();
    }
